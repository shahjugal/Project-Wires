...
import datetime
load_dotenv()
SECRET_TOKEN_KEY = os.environ.get("SECRET_VERIFY_KEY")

class VerificationKeyException(Exception):
    def __init__(self, code: int, message: str):
        self.code = code
        self.message = message

class VerificationKeyUtility:

    @staticmethod
    def generate_key(user_id: int, for_duration: datetime = datetime.timedelta(hours=2)) -> str:
