...
class TweetUtil:
    ...
    @staticmethod
    def delete_tweet(db: Session, user_id: int, tweet_id: int) -> bool:
        tweet = db.query(Tweet).filter_by(id=tweet_id, user_id=user_id).first()
        if tweet:
            db.delete(tweet)
            db.commit()
            return True
    ...
    @staticmethod
    def update_tweet(db: Session, user_id: int, tweet_id: int, new_content: str) -> bool:
        tweet = db.query(Tweet).filter_by(id=tweet_id, user_id=user_id).first()
        if tweet:
    ...
    @staticmethod
    def retweet_tweet(db: Session, user_id: int, tweet_id: int) -> int:
        new_retweet = Retweet(user_id=user_id, tweet_id=tweet_id)
        db.add(new_retweet)
        db.commit()
        db.refresh(new_retweet)
        return new_retweet.id
    
    @staticmethod
    def un_Retweet(db: Session, user_id: int, tweet_id: int) -> bool:
        existing_like = db.query(Like).filter_by(user_id=user_id, tweet_id=tweet_id).first()
        if existing_like:
            db.delete(existing_like)
            db.commit()
